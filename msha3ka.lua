...
chat_kick(msg.chat_id_, msg.sender_user_id_)
     ...
     local id = msg.id_
        ...
        local user_id = msg.sender_user_id_
        delete_msg(chat,msgs)
          send(msg.chat_id_, 0, 1, "<code>ايديك : </code><i>"..msg.sender_user_id_.."</i>\n<code>تخيل اخليك ترسل روابط 🌝👅</code>", 1, 'html')
	end
 end
...
if database:get('bot:forward:warn'..msg.chat_id_) then
	if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
     local id = msg.id_
        ...
        local user_id = msg.sender_user_id_
        delete_msg(chat,msgs)
          send(msg.chat_id_, 0, 1,  "<code>ايديك : </code><i>"..msg.sender_user_id_.."</i>\n<code>تخيل اخليك سوي تۄچيهہ 🌝👅</code>", 1, 'html')
	end
   end
end
elseif msg_type == 'MSG:Text' then
   ...
   if database:get('bot:tag:mute'..msg.chat_id_) then
    ...
    local user_id = msg.sender_user_id_
       delete_msg(chat,msgs)
          send(msg.chat_id_, 0, 1, "<code>ايديك : </code><i>"..msg.sender_user_id_.."</i>\n<code>التاكات <#> تم قفلها مم��وع ارسالها</code>", 1, 'html')
          return 
   end
end
...
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
    ...
    if database:get('lang:gp:'..msg.chat_id_) then
	  ...
	  if unmutept[2] == "service" or UNTSHAKE[2] == "الدخول بالرا��ط" then
	  if database:get('bot:tgservice:mute'..msg.chat_id_) then
    if database:get('lang:gp:'..msg.chat_id_) then
...
.."`انقضاء البوت` : "..exp_dat.."\n" .."======================"
	...
	-----------------------------------------------------------------------------------------------
  	if text:match("^[Dd][Ee][Vv]$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
       sendContact(msg.chat_id_, msg.id_, 0, 1, nil, 9647731928959, 'يآسِر حسْن ➺┋☻', '', bot_id)
    end
	-----------------------------------------------------------------------------------------------
  ...
  local matches = {string.match(text, "^(تنظيف) (%d+)$")}
   ...
   offset_ = 0,
      ...
      pm ='> <i>'..matches[2]..'</i> <code>م�� الرسائل تم حذفها</code>'
           send(msg.chat_id_, msg.id_, 1, pm, 1, 'html')
       end
...
if text:match("^معلوماتي$") or text:match("^معلوماتي$") and msg.reply_to_message_id_ == 0 then
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
...
*======================*
*h3* `لعرض اوامر الادمنيه`
*======================*
*h4* `لعرض اوامر المجموعه`
*======================*
*h5* `لعرض اوامر المطورين`
*======================*
]]
                send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
   end
...
*======================*
   ...
   if text:match("^[Hh]5") and is_sudo(msg) then
...
*======================*
   ...
   if text:match("^م5") and is_sudo(msg) then
...
<code>ياسرحسن</code>
   ...
   if text:match("^@ll303ll$") or text:match("^ياسر$") and msg.reply_to_message_id_ == 0 then
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
          function get_me(extra,result,success)
      if is_sudo(msg) then
      if database:get('lang:gp:'..msg.chat_id_) then
      t = 'المطور مآلتي فديتهہ🌝👅 '..'\n  '..' Dev | @Y3SOO'
      else
      t = 'المطور مآلتي فديتهہ🌝👅 '..'\n  '..' Dev | @Y3SOO'
...
send(msg.chat_id_, msg.id_, 1, "\n  "..t.."\n ", 1, 'rrr')
   ...
   if text:match("^بوت$") or text:match("^بوت$") and msg.reply_to_message_id_ == 0 then
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
          function get_me(extra,result,success)
      if is_sudo(msg) then
      if database:get('lang:gp:'..msg.chat_id_) then
...
send(msg.chat_id_, msg.id_, 1, "\n  "..t.."\n ", 1, 'rrr')
   ...
   if text:match("^كوكي$") or text:match("^كوكي$") and msg.reply_to_message_id_ == 0 then
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
          function get_me(extra,result,success)
      if is_sudo(msg) then
      if database:get('lang:gp:'..msg.chat_id_) then
      t = '\n'..'يـآ بـعد رووح كـؤكـي 🌝👅'..'\n'..'مـشـتـغـل والـلـه 🤖🤘'
      else
      t =         '\n'..'يـآ بـعد رووح كـؤكـي 🌝👅'..'\n'..'مـشـتـغـل والـلـه 🤖🤘'
...
send(msg.chat_id_, msg.id_, 1, "\n  "..t.."\n ", 1, 'rrr')
   ...
   if text:match("^هلو$") or text:match("^هلاو$")     or text:match("^مرحبا$")    or text:match("^السلام عليكم$")    or text:match("^هاي$")  and msg.reply_to_message_id_ == 0 then
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
          function get_me(extra,result,success)
      if is_sudo(msg) then
      if database:get('lang:gp:'..msg.chat_id_) then
